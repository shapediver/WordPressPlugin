...
class ConfiguratorManager implements IConfiguratorManager {
  ...
  constructor() {
    this.runsInsideECommerceSystem =
      document.querySelector(ECOMMERCE_SELECTOR) !== null;
  ...
  async loadConfigurator(
    ...
    if (!productId) {
      ...
      if (element) productId = (element as HTMLButtonElement).value;
    }
    if (!productId) {
  ...
  enableConfigurator(): void {
    ...
    if (!openConfiguratorButton) {
      this.log(
        `ConfiguratorManager: Element with id ${OPEN_CONFIGURATOR_BUTTON_ID} not found.`
      );
    } else {
